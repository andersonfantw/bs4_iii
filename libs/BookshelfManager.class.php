...
*/
  ...
  function doEcocatUpdate($type,$bs_code,$uid,$cid='',$processid='',$bsss_id=''){
		...
		foreach($ecocat as $key=>$val)
		{
		  ...
		  //ecocat有，DB有(之前曾經從ecocat匯入過)且上架中 => DB內此本書籍維持上架不變
		  //ecocat有，DB有(之前曾經從ecocat匯入過)且下架中 => DB內此本書籍維持下架不變
		  ...
		  }else{  //ecocat有，DB沒有 => 新增書籍至DB且設定為上架
		    $data['b_status'] = '1';
		    if($type=='share_bs'){
		    	//分享的書櫃ecocat_id用來記錄bsss_id
		...
		//ecocat沒有(下架了)，DB有(之前曾經從ecocat匯入過)且上架中 => DB內此本書籍要變成下架
		//ecocat沒有(下架了)，DB有(之前曾經從ecocat匯入過)且下架中 => DB內此本書籍維持下架不變
