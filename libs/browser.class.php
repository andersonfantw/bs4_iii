...
class browser{
public function detect( $which_test, $test_excludes='', $external_ua_string='' ) 
{
	/*
...
*/
function BroserDetectionRedirect(){
	//如果是手持裝置
	header("HTTP/1.1 307 Temporary Redirect");
	switch(browser_detection('ua_type')){
		case 'bot':		//(web bot)
	    if(browser_detection('ua_type')!='bot'){
			  header("Location: ".WEB_URL."/desktop/");
	    }
			break;
		case 'bro':		//(normal browser)
	    if(browser_detection('ua_type')!='bro'){
			  header("Location: ".WEB_URL."/desktop/");
	    }
			break;
		case 'bbro':	//(simple browser)
	    if(browser_detection('ua_type')!='bbro'){
			  header("Location: ".WEB_URL."/desktop/");
	    }
			break;
		case 'mobile':	//(handheld)
			//所有的手機版本都先導入到iPhone的畫面，其他的都導入到一般書櫃頁面
	    if(browser_detection('ua_type')!='mobile'){
			  header("Location: ".WEB_URL."/iphone/");
	    }
			break;
		case 'dow':		//(downloading agent)
	    if(browser_detection('ua_type')!='dow'){
			  header("Location: ".WEB_URL."/desktop/");
	    }
			break;
		case 'lib':		//(http library)
	    if(browser_detection('ua_type')!='lib'){
		  header("Location: ".WEB_URL."/desktop/");
	    }
			break;	
	}
