...
$(function() {
    ...
    $("#q").bind('keyup click', function() { 
        $.post("sys_infoacer.php?type=search_instant", //post 
            $("#search").serialize(),  
                function(data){ 
...
<!--
				<div>
						<form action="sys_infoacer.php?type=search" method="post" class="forms" name="form" id="search">
							<ul>
								<li>
									<label  class="desc">
...
-->
					<div class="other">						
						<div class="button float-right">
							<a href="sys_allexam.php" class="btn ui-state-default"><span class="ui-icon ui-icon-circle-plus"></span><{$smarty.const.LANG_INFOACER_LIST_BTN_BACK}></a>
						</div>
					...
					<table cellspacing="0">
						...
						<{foreach from=$data item="val" name=myloop}>   
							<tr<{if $smarty.foreach.myloop.iteration is even}> class="alt"<{/if}>>
								...
								<{/foreach}>
								<td>
									<a class="btn_no_text btn ui-state-default ui-corner-all tooltip delete" title="<{$smarty.const.LANG_BUTTON_DELETE}>" href="sys_infoacer.php?type=delete&id=<{$val.key}>&page=<{$smarty.get.page}>">
