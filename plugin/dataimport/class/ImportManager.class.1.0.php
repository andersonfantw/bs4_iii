...
class ImportManager extends ExportManager{
	...
	var $_ValidExpression = array(
		...
		'account'=>'/^([a-zA-Z][\w]{5,15})$/',	//less 6 character, 20 character max. start and emd with a-zA-Z, a-zA-Z_ in between.
		'password'=>'/^(.{6,20})$/',	//any character between 6 and 20 times
	...
	'en' => array(
		...
		'account'=>'Account must between 6~20, start with letter',
		'password'=>'Passeord must between 6~20 any character',
...
ecocat_id=''
and share_bs_id=''
and ((webbook_link LIKE 'http://127.0.0.1%/files/%' or webbook_link NOT LIKE 'http://127.0.0.1%')
			or (ibook_link LIKE 'http://127.0.0.1%/files/%' OR ibook_link NOT LIKE 'http://127.0.0.1%'))
SQL;
				$data = $book->getList('',0,0,$condition.$_condition);
...
SQL;
									...
									$str = implode(',',array_values($arr['c_id']));
