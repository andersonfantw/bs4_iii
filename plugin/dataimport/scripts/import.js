...
var ImportAction = function(panel){
	$('#body').prepend(panel);

	var _convert_allowfiletype;
	var _convert_filetypes = {'book':'.zip',
														'cate':'.xls',
														'user':'.xls',
														'group':'.xls',
														'admin':'.xls'};

	ImportAPIHandler.getStatusCode('statuscode',systemEnv.isBackend,function(_convert_status_code){
