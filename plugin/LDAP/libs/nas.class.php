...
class Nas
{
    ...
    var $domaintype = LDAP_DOMAINTYPE;
    ...
    public function login($username, $password)
    {
		...
		#Ckeck token, see if valid
		public function isValid($authSid)
		{
        $pageContents = HttpClient::quickPost(
            "http://127.0.0.1:".$this->nasPort."/cgi-bin/sys/sysRequest.cgi",
            array(
                "sid" => $authSid
            )
        );
        ...
        #Check valid
	$xml = simplexml_load_string($pageContents, 'SimpleXMLElement', LIBXML_NOCDATA);
        $json = json_encode($xml);
	$arr = json_decode($json);
        return $arr->authPassed;
		}
