...
var TagAPIHandler ={
  ...
  getTagMap : function(returnFunc){
	...
	validTagByPKey : function(_pkey,returnFunc){
    var _msgHadler = new MsgAction('TagAPIHandler');
    $.ajax({
        type: "post",
        dataType: "text",
        url: this.getAPIurl("getDropDownListItems"),
        data: {method:'validByPKey',pkey:_pkey},
        success: function(data){
            _msgHadler.Log('validByPKey data='+JSON.stringify(data));
            _msgHadler.Log('Reveive validByPKey success!');
						if(!StorageHandler.getTagStr(_pkey)){
            	returnFunc(false);
            }else{
            	returnFunc(ToolsHandler.MD5(lzw_decode(StorageHandler.getTagStr(_pkey)))==data);
            }
        },
        error: function(){
            _msgHadler.Log('Reveive validByPKey fail!');
        },
        async:true
    });
	},
  getTagByPKey : function(_pkey,returnFunc){
    var _msgHadler = new MsgAction('TagAPIHandler');
    TagAPIHandler.validTagByPKey(_pkey,function(v){
    	if(v){
    		_val=$.parseJSON(lzw_decode(StorageHandler.getTagStr(_pkey)));
    		returnFunc(_val);
    	}else{
	      $.ajax({
	          type: "post",
	          dataType: "json",
	          url: TagAPIHandler.getAPIurl("getDropDownListItems"),
	          data: {method:'getByPKey',pkey:_pkey},
	          success: function(data){
            		_val = lzw_encode(JSON.stringify(data));
            		StorageHandler.setTagStr(_pkey,_val);
	              _msgHadler.Log('getByPKey data='+JSON.stringify(data));
	              _msgHadler.Log('Reveive getByPKey success!');
	              returnFunc(data);
	          },
	          error: function(){
	              _msgHadler.Log('Reveive getByPKey fail!');
	          },
	          async:true
	      });
    	}
	...
	validAllTagByPKey : function(_pkey,returnFunc){
    var _msgHadler = new MsgAction('TagAPIHandler');
    $.ajax({
        type: "post",
        dataType: "text",
        url: this.getAPIurl("getDropDownListItems"),
        data: {method:'validAllTagByPKey',pkey:_pkey},
        success: function(data){
            _msgHadler.Log('validAllTagByPKey data='+JSON.stringify(data));
            _msgHadler.Log('Reveive validAllTagByPKey success!');
						if(!StorageHandler.getTagStr(_pkey+'_all')){
            	returnFunc(false);
            }else{
            	returnFunc(ToolsHandler.MD5(lzw_decode(StorageHandler.getTagStr(_pkey+'_all')))==data);
            }
        },
        error: function(){
            _msgHadler.Log('Reveive validAllTagByPKey fail!');
        },
        async:true
    });
	},
  getAllTagByPKey : function(_pkey,returnFunc){
    var _msgHadler = new MsgAction('TagAPIHandler');
    TagAPIHandler.validAllTagByPKey(_pkey,function(v){
    	if(v){
    		_val=$.parseJSON(lzw_decode(StorageHandler.getTagStr(_pkey+'_all')));
    		returnFunc(_val);
    	}else{
	      $.ajax({
	          type: "post",
	          dataType: "json",
	          url: TagAPIHandler.getAPIurl("getDropDownListItems"),
	          data: {method:'getAllTagByPKey',pkey:_pkey},
	          success: function(data){
            		_val = lzw_encode(JSON.stringify(data));
            		StorageHandler.setTagStr(_pkey+'_all',_val);
	              _msgHadler.Log('getAllTagByPKey data='+JSON.stringify(data));
	              _msgHadler.Log('Reveive getAllTagByPKey success!');
	              returnFunc(data);
	          },
	          error: function(){
	              _msgHadler.Log('Reveive getAllTagByPKey fail!');
	          },
	          async:true
	      });
    	}
  ...
  importByStr : function(_str,_m,returnFunc){
    var _msgHadler = new MsgAction('ChartAPIHandler');
    $.ajax({
        type: "post",
        dataType: "json",
        url: this.getAPIurl("importByStr"),
        data: {bs:systemEnv.bsid,m:_m,str:_str},
        success: function(data){
            _msgHadler.Log('importByStr data='+JSON.stringify(data));
            _msgHadler.Log('Reveive importByStr success!');
            returnFunc(data);
        },
        error: function(){
            _msgHadler.Log('Reveive importByStr fail!');
        },
        async:true
    });  	
  }
