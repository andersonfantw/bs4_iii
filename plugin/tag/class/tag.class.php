...
SQL;
		...
		foreach($data as $row){
			...
			$sql=<<<SQL
select t_id,key,val,path,type
from bookshelf2_view_tags_minall vt
where t_id in (%s)
order by t_id desc
SQL;
			$_path = $row['path'];
...
SQL;
	...
	public function getAllTagByPKey($pkey,$is_min=false){
			$sql=<<<SQL
select t_id,key,val,path,type
from BOOKSHELF2_VIEW_TAGS_MINALL
where ifnull(pkey,'')='%s'
order by val
SQL;
	...
	public function getTagByPKey($pkey,$is_min=false){
		if($is_min){
			$sql=<<<SQL
select t_id,key,val,path,type
from BOOKSHELF2_VIEW_TAGS_MINALL
where ifnull(pkey,'')='%s' and isactive=1
order by val
SQL;
		}else{
			$sql=<<<SQL
...
SQL;
	...
	private function _getTag($kid,$vid,$parentid){
		...
		$where = sprintf('isActive=1 and tk_id=%u and tv_id=%u and pid=%u',$kid,$vid,$parentid);
		$data = $this->getList('',0,0,$where);
		if($data['total']==0){
			return 0;
		}else{
...
SQL;
	...
	function rename($tid,$name){
		$sql=<<<SQL
update bookshelf2_tagval set tv_name='%s'
where tv_id=(select tv_id from bookshelf2_tag where t_id=%u)
SQL;
		$sql=sprintf($sql,$name,$tid);
