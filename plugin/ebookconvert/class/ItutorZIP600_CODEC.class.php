...
class ItutorZIP600_CODEC implements iBSFile_CODEC{
	const Name='itutor600';
	const ExtensionName='zip';

	public function getDataFolders(){
		return array('/data'=>'png,jpg,jpeg,mp3,mp4,ogg,webm',
									'/player/images/'=>'png');
	}
	
	public function getDataFiles(){
		return array('/player/rlparams.js'=>'/player',
			'/player/rlplayer.js'=>'/player',
			'/player/rlslides.js'=>'/player',
			'/player/rlplayer.css'=>'/player');
	}
	...
	public function CheckFormat($tmpfolder){
		$exist = array('/player/rllanguages.js');
		$f_exist=0;
		foreach($exist as $file){
			if(is_file($tmpfolder.$file)){
				/*
				$f1 = filesize(PLUGIN_PATH.sprintf(BSFile_CODEC::sysDataPath,self::Name).$file);
				$f2 = filesize($tmpfolder.$file);
				if($f1!=$f2){
					$f_exist=0;
				}
				*/
				$handle = fopen($tmpfolder.$file, "r");
				if($handle){
					fgets($handle, 255);
					$line = fgets($handle, 255);
					$f_exist=(strpos($line,'Generated by iTutor 6.0.0')!==false);
				}
				fclose($handle);
			}
		}
		if($f_exist){
			$this->zipFormat = ConvertModeEnum::ITUTOR600_ZIP;
	...
	public function postDecode($sourcepath,$destpath){
		...
		//set report url
		$report_path = $destpath.'/player/rlparams.js';
