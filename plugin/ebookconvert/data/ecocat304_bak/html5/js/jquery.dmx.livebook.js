...
(function($) {
    ...
    var getPageLabel = function (page) {
    	return page + settings.disp_stpage_num + settings.disp_stpage_cnt;
    };
...
//            methods.createzoommovebutton.apply($this, [data.target]);
                ...
                if (status.flipping === false && status.sliding === false && status.changespread === false) {
                    ...
                    if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) { //見開きの場合
                        ...
                        switch (settings.pageaction)
                        {
                            ...
                            case DEF_PARAMS.PAGEACTION.RSLIDE:
                                if (d === 'left') {
                                    methods.flipstart.apply($this, [data.target, status.curbasepage + 1]);
                                } else {
                                    methods.flipstart.apply($this, [data.target, status.curbasepage]);
                                }
            ...
            indexlist_gotopage_fnc = function(e){
                if (status.flipping === false && status.sliding === false && status.changespread === false) {
                    ...
                    if (t == DEF_PARAMS.LINKTYPE.PAGE) {
                        ...
                        if (status.zoommode) {
                            params.delayfuncparam = [$(selectors.livebook), $(e.currentTarget).data('link')];
            ...
            $(document).on("vclick", '#controllbar #l_top', function(e){
            	...
            	if (status.flipping === false && status.sliding === false && status.changespread === false) {
	            	if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) { //見開きの場合
	                    status.flipping = true;
	                    methods.gotostart.apply($this, [data.target, 0]);
	                } else {
	                    status.sliding = true;
		            	if (is_blankpage(0, params.lastpage, settings.h0_invisible, settings.h5_invisible)) {
		            		methods.gotostart.apply($this, [data.target, 1]);
		            	} else {
		            		methods.gotostart.apply($this, [data.target, 0]);
		            	} 
	                }
            ...
            $(document).on("vclick", '#controllbar #l_page', function(e){
            	...
            	if (status.flipping === false && status.sliding === false && status.changespread === false) {
            		if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) {
            			$(selectors.pageflipbuttonl).trigger('dmxtap');
            		} else {
            			switch (settings.pageaction) {
	                        case DEF_PARAMS.PAGEACTION.RIGHT:
	                        case DEF_PARAMS.PAGEACTION.UPPER:
	                        case DEF_PARAMS.PAGEACTION.RSLIDE:
	                            methods.gotostart.apply($this, [data.target, status.curbasepage + 1]);
            ...
            $(document).on("vclick", '#controllbar #r_page', function(e){
            	...
            	if (status.flipping === false && status.sliding === false && status.changespread === false) {
            		if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) {
            			$(selectors.pageflipbuttonr).trigger('dmxtap');
            		} else {
	                    switch (settings.pageaction) {
	                        case DEF_PARAMS.PAGEACTION.RIGHT:
	                        case DEF_PARAMS.PAGEACTION.UPPER:
            ...
            $(document).on("vclick", '#controllbar #r_top', function(e){
            	...
            	if (status.flipping === false && status.sliding === false && status.changespread === false) {
	            	if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) { //見開きの場合
	                    status.flipping = true;
	                    methods.gotostart.apply($this, [data.target, params.lastpage]);
	                } else {
	                    status.sliding = true;
		            	if (is_blankpage(params.lastpage, params.lastpage, settings.h0_invisible, settings.h5_invisible)) {
		            		methods.gotostart.apply($this, [data.target, params.lastpage - 1]);
		            	} else {
		            		methods.gotostart.apply($this, [data.target, params.lastpage]);
		            	} 
	                }
            	...
            	if (!isNaN(goto_text)) {
            		...
            		if (0 <= goto_num && goto_num <= params.lastpage) {
		            	if (status.flipping === false && status.sliding === false && status.changespread === false) {
			                ...
			                methods.gotostart.apply($this, [data.target, goto_num]);
			                if (settings.device == 'pc') {
			                	$('#goto_text').val('');
			                }
		            	}
...
//            	methods.normalizememo.apply($this, [$this, $(e.target).parents('.memo_area')]);
            ...
            $(document).on("vclick", 'div.tag_area span', function(e){
                ...
                if (page_num != s_page_num && page_num != s_page_num + 1) {
                	methods.gotostart.apply($this, [$this, page_num]);
                }
...
//                    methods.resetsearchallposition.apply($this, [$this]);	//@ToDo
            	...
            	if (!$(this).hasClass('disabled')) {
	                ...
	                switch (settings.pageaction)
	                {
	                    ...
	                    case DEF_PARAMS.PAGEACTION.RSLIDE:
	                        ...
	                        //現在のページ
	                    	$('#printarea').append('<div class="printpage"><img src="'+params.pageimgdir+params.pageimgprefix+'__dmx__' + (status.curbasepage + 1) + '__1000.jpg"></div>');
	                    ...
	                    case DEF_PARAMS.PAGEACTION.LSLIDE:
	                        ...
	                        //現在のページ
	                    	$('#printarea').append('<div class="printpage"><img src="'+params.pageimgdir+params.pageimgprefix+'__dmx__' + status.curbasepage + '__1000.jpg"></div>');
            ...
            $(document).on("vclick", '#controllbar #help', function(e){
            	$('#help_screen').append('<div role="dialog" class="ui-dialog-contain ui-overlay-shadow ui-corner-all"><div class="help_content"></div><div class="help_close_btn"></div></div>');
            });
            $(document).on("vclick", '#help_screen .help_close_btn', function(e){
        ...
        setpagenum: function(){
            ...
            if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) { //見開きの場合
                switch (settings.pageaction) {
                    ...
                    case DEF_PARAMS.PAGEACTION.RSLIDE:
                		...
                		tp = params.lastpage - settings.disp_stpage_num + settings.disp_stpage_cnt;
            ...
            if (cp < settings.disp_stpage_cnt) {
            	cp = '-';
            }
            if (settings.device == 'pc') {
        ...
        zoominstart: function (){
            ...
            if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) { //見開きの場合
                ...
                if (!status.zoommode) {
	                if ($(selectors.zoompages+'[rel='+lp+']', t).length === 0) methods.addpage.apply($(this), [$(selectors.zoom_frame), lp, 400]);
	                if ($(selectors.zoompages+'[rel='+rp+']', t).length === 0) methods.addpage.apply($(this), [$(selectors.zoom_frame), rp, 400]);
                ...
                if (!status.zoommode) {
                	if ($(selectors.zoompages+'[rel='+p+']', t).length === 0) methods.addpage.apply($(this), [$(selectors.zoom_frame), p, 400]);
        ...
        zoominaction: function () {
            ...
            a2.complete = function(){
                if ($(selectors.zoomleftpage, t).length === 1 && $(selectors.zoomleftimgz, t).length === 0) methods.overlayzoompage.apply($(this), [$(selectors.zoomleftpage), 1000, w, h]);
                if ($(selectors.zoomrightpage, t).length === 1 && $(selectors.zoomrightimgz, t).length === 0) methods.overlayzoompage.apply($(this), [$(selectors.zoomrightpage), 1000, w, h]);
        ...
        tracezoomactionterminate: function () {
            ...
            if (params.zscale.tscale < params.pscale) {
	            ...
	            a2.complete = function(){
	            	if ($(selectors.zoomleftpage, t).length === 1 && $(selectors.zoomleftimgz, t).length === 0) methods.overlayzoompage.apply($(this), [$(selectors.zoomleftpage), 1000, params.zoom_pw, params.zoom_ph]);
	                if ($(selectors.zoomrightpage, t).length === 1 && $(selectors.zoomrightimgz, t).length === 0) methods.overlayzoompage.apply($(this), [$(selectors.zoomrightpage), 1000, params.zoom_pw, params.zoom_ph]);
        ...
        slidedragterminate: function () {
            ...
            if (0 < dx) {
                methods.slidestart.apply($(this), [t, 'r2l', md]);
            } else if (dx < 0) {
                methods.slidestart.apply($(this), [t, 'l2r', md]);
            } else {
        ...
        addpage: function() {
            ...
            t.append('<div class="page ' + d + '" rel="' + p + '" style="z-index: ' + z + '; display: ' + v + '"><img src="'+imgsrc+'"></div>');
        },
        gotostart: function() {
            var t = arguments[0]; //target
            ...
            if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) //見開きの場合
            {
                ...
                //ページの有無確認と、なければ追加（プリロード用）
                if ($(selectors.pages+'[rel='+plnp+']', t).length === 0) methods.addpage.apply($(this), [$(selectors.paper_frame), plnp, 400]);
                if ($(selectors.pages+'[rel='+plpp+']', t).length === 0) methods.addpage.apply($(this), [$(selectors.paper_frame), plpp, 400]);
                ...
                params.timerid = setTimeout(methods.slideaction, params.slidedelay, $(this), [t, DEF_PARAMS.SLIDEMODE.SLIDE, cp, np, null]);
            }
        },
        flipstart: function() {
            var t = arguments[0]; //target
        ...
        slidestart: function() {
            ...
            if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) //見開きの場合
            {
                ...
                params.slidedirection = d;
                params.timerid = setTimeout(methods.slideaction, 0, $(this), [t, m, clp, crp, nlp, nrp, plp, prp]);
            } else {
                // めくり方向の判定
                switch (settings.pageaction)
                {
                ...
                params.slidedirection = d;
                params.timerid = setTimeout(methods.slideaction, 0, $(this), [t, m, cp, np, pp]);
            }
        },
            ...
            if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) { //見開きの場合
                ...
                if (m === DEF_PARAMS.SLIDEMODE.REWIND) {
                    aop2.complete = function () {
                        ...
                        methods.slideterminate.apply($(this), [t, Math.min(clp, crp)]);
                    };
                ...
                if (m === DEF_PARAMS.SLIDEMODE.REWIND) {
                    aop2.complete = function () {
                        ...
                        methods.slideterminate.apply($(this), [t, cp]);
                    };
        ...
        changespread: function(){
            ...
            if (status.spreadmode === DEF_PARAMS.SPREADMODE.SINGLE &&
                ...
                if ($(selectors.pages+'[rel='+nlp+']', $this).length === 0) methods.addpage.apply($this, [$(selectors.paper_frame), nlp, 400]);
                if ($(selectors.pages+'[rel='+nrp+']', $this).length === 0) methods.addpage.apply($this, [$(selectors.paper_frame), nrp, 400]);
                if ($(selectors.pages+'[rel='+plp+']', $this).length === 0) methods.addpage.apply($this, [$(selectors.paper_frame), plp, 400]);
                if ($(selectors.pages+'[rel='+prp+']', $this).length === 0) methods.addpage.apply($this, [$(selectors.paper_frame), prp, 400]);
                ...
                if ($(selectors.pages+'[rel='+np+']', $this).length === 0) methods.addpage.apply($this, [$(selectors.paper_frame), np, 400]);
                if ($(selectors.pages+'[rel='+pp+']', $this).length === 0) methods.addpage.apply($this, [$(selectors.paper_frame), pp, 400]);
