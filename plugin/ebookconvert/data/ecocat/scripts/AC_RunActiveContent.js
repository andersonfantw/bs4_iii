...
function openPopupWindow (url, name, feature) {
	var winObj = window.open(url, name, feature);
	winObj.focus();
}
...
function openBook(myURL)
{
	window.open(myURL, "", "width=1010,height=655,resizable=yes,scrollbars=no,status=no,menubar=no,location=no,toolbar=no");
}

function displayCsv(page, data, color) {

    // JQueryの呼び出し
    //document.write("<script type='text/javascript' src='../book/scripts/jquery-1.3.2.js'></script>");

    if (color != '') {
        source_file_name = "../php_module/web/php/memo.php";
    } else {
        source_file_name = "../php_module/web/php/tag.php";
    }

    //alert(source_file_name);

    // 非同期通信開始
    $.post(source_file_name, { "page": page, "data": data, "color": color }, callback_func );
...
function callback_func(data) {
    ...
    // ファイル名の取得
    window.open('../php_module/web/php/download.php?url=' + data, 'tagDownload', 'width=400, height=300, menubar=no, toolbar=no, scrollbars=no location=no status=no resizable=no');
}
