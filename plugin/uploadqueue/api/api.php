...
switch($cmd){
	case 'pp':
		$EcocatConnector = new EcocatConnector(5);
		$result = $EcocatConnector->Process('edae4b2c_7d742b6d','1585818490');
var_dump($result);
		break;
	case 'login':
...
*/
	...
	case 'update2':
		...
		$tags = $fs->valid($_POST['tags'],'content');
		$arr = explode(';',$tags);
...
/*
	case 'iiissotest':
		...
		$arr = array('code'=>'0','unit'=>'pcu31','account'=>'demo','username'=>'demo');
...
*/
	case 'iiisso':
		...
		$iiiurl = 'https://doitpms.ctdp.org.tw/content/application/doitpms/api/ebookServer.php';
		...
		$body = curl_exec($ch);
//var_dump($iiiurl,$iiipostjson,$body,curl_version());
//var_dump(curl_errno($ch),curl_error($ch));
		...
		$TagTree->loadDB();
/*
		$account = $fs->valid($_POST['acc'],'acc');
		$username=$account;
*/
		//see if user exist
...
file_put_contents('/var/www/html/logs/logincheck.log',sprintf("%s=uploadqueue/api\nsession_id=%s\n",date('Y:m:d H:i:s'),session_id()),FILE_APPEND);
file_put_contents('/var/www/html/logs/logincheck.log',sprintf("line486=SESSIOIN:%s\n",$ss),FILE_APPEND);
			...
			if(!empty($val)){
				...
				switch($type){
					...
					default:
						switch($_name){
							case 'pn':
								$TagSearch->addConditionByTagKeyword($val,$_name);
								break;
/*
							case 'prt':
							case 'pwrf':
...
*/
							default:
								...
								if(is_array($val)){
									foreach($val as $v){
										$TagSearch->addConditionByTID($_name,$v);
									}
								}else{
	  ...
	  $json = new Services_JSON(SERVICES_JSON_ESCAPED_UNICODE);
		...
		break;
	case 'addBookshelf':
		$ecocatid = '0ef79347_9520835a';
		$timestamp = '1578367922';
		$filename = 'doitpms_277103120200107113046.pdf';
		$key = '4745998';
		$UploadQueue = new UploadQueue();
		$UploadQueue->setBSID(3);
		$bid = $UploadQueue->addToBookshelf($ecocatid,$timestamp,$filename,$key);
		var_dump($bid);
		break;
}
