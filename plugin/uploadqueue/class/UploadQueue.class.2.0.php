...
var_dump($qid,$row['q_name'],$this->tagstr);
						$this->_mail(QueueStatusEnum::Fail,$qid,$row['q_name'],$this->tagstr);
					}
...
*/
	...
	private function _mail($status,$qid,$name,$tagstr){
		print_r("\ncall send mail");
		...
		$mail->Subject = sprintf('[數位圖書館]檔案轉換失敗通知 - %s',$name);
