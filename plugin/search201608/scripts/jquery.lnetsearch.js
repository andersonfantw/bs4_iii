...
$.fn.lnetsearch = function(options){
	...
	json = {year_year:{name:'年度',controller:'select|select',class:'chosen',type:'period',year:{name:'年度',server:true,value:{thisyear:'今年',lastyear:'去年'}}},
					pn:{name:'計畫名稱',controller:'input',class:'input'},
					py_pty:{name:'計畫年數',controller:'select|select',class:'chosen',type:'of',py:{name:'計畫年數',server:true},pty:{name:'計畫總年數',server:true}},
	...
	function _init(){
		...
		APIHandler.buinfo(function(data){
			if(data.type=='u'){
				...
				//load and update localstorage search item setting
	      		...
	      		orderable: false,
						"render": function(data, type, row, meta){
					  	if(type === 'display'){
					    	...
					    	//data = '<a href="javascript:SearchAPIHandler.MM_openBrWindow(\'' + _link1 + '\');">開啟</a> &nbsp; <a href="javascript:SearchAPIHandler.MM_openBrWindow(\'' + _link2 + '\');">下載</a>';
					    	data = '<a href="javascript:SearchAPIHandler.MM_openBrWindow(\'' + _link1 + '\');">開啟</a>';
					  	}
					  	return data;
						}
	      	},
...
console.log('setQuickSearchList');
...
console.log('return');
		...
		settings._searchsetting = StorageHandler.getSearchItemSettingStr(_buid);
console.log(settings);
		if(settings._searchsetting){
			settings._searchsetting=JSON.parse(settings._searchsetting);
		}else{
			settings._searchsetting=[];
		}
...
*/
						...
						//especially customize for ROC year
						//second number must greater than first
						if(!_year1_change_event){
							_year1_change_event=true;
							$searchpanel.find('.year1').on('change',function(){
								_val = $(this).find('option:selected').val();
								_text = $(this).find('option:selected').text();
								_setYear2Option(_val,_text);
							});
						}
	...
	function _setYear2Option(_opt1val,_opt1text,_opt2val){
		_opt2val = _opt2val || '';
		k='year_year';
		pk = k.split('_');
		_k1=pk[0];
		...
		switch(_opt1val){
			case 'thisyear':
				...
				$searchpanel.find('.'+_k2).prop('disabled', true).trigger("chosen:updated");
				break;
			case 'lastyear':
				//leave only thisyear
				_options2 += _template_option.replace('@key@','thisyear').replace('@val@',json[k]['year'].value['thisyear']);
				$searchpanel.find('.'+_k2).empty();
				$searchpanel.find('.'+_k2).append(_options2);
				$searchpanel.find('.'+_k2).prop('disabled', false).val(_opt2val).trigger("chosen:updated");
				break;
			default:
				switch(_opt1text){
					case _thisyear-1911://thisyear
						//disable second select
						_options2 = _template_option.replace('@key@','thisyear').replace('@val@',json[k]['year'].value['thisyear']);
						break;
					case _thisyear-1912://lastyear
						//leave only thisyear
						for(var j in json[k][pk[1]].value){
							_options2+=_template_option.replace('@key@',j).replace('@val@',json[k][pk[1]].value[j]);
						}
						break;
					default:
						//leave the year less than selected
						exclude = [];
						if(_opt1text>_thisyear-1912){
							exclude.push('lastyear');
						}
						if(_opt1text>_thisyear-1911){
							exclude.push('thisyear');
						}
						for(var j in json[k][pk[1]].value){
	...
	function _setQueryItemComplete(_buid){
		if(Object.keys(json).length-Object.keys(settings._searchsetting).length==$searchpanel.find('.right-content>div>.search-item>ul>li').length){
			...
			$searchpanel.find('.right-content>div>.search-item .save').click(function(){
				...
				_condition = getquerystring('save');
...
console.log(_min);
	...
	***********************************/
	function _setQueryValue(_buid,_condition){
		...
		if(_condition){
			settings._searchsetting = StorageHandler.getSearchItemSettingStr(_buid);
			if(!settings._searchsetting) settings._searchsetting=[];
console.log(settings);
...
console.log(_index);
							...
							break;
						case 'select|select':
							switch(json[_hash[k]].type){
								case 'period':
									if($.isArray(_condition[k].v)){
										_v1 = _condition[k].v[0];
										_v2 = _condition[k].v[1];
									}else{
										_v1 = _condition[k].v;
										_v2 = '';
									}
									$(window[k+'1'].selector).val(_v1).trigger("chosen:updated");
									_setYear2Option(_v1,_condition[k].t[0],_v2);
									//$(window[k+'1'].selector).trigger('change');
									//$(window[k+'2'].selector).val(_condition[k].v[1]).trigger("chosen:updated");
									break;
								default:
									$(window[k].selector).val(_condition[k].v).trigger("chosen:updated");
									break;
										...
										case 'period':
											...
											switch(_mode){
												case 'save':
													if(window[_k1].val() && window[_k1].val()!='-'){
														if(window[_k2].val() && window[_k2].val()!='-'){
															_r[pk[1]] = {n:json[k][pk[1]].name,class:json[k].class,type:json[k].type,v:[window[_k1].val(),window[_k2].val()],t:[$(window[_k1].selector).find('option:selected').text(),$(window[_k2].selector).find('option:selected').text()]};
														}else{
												...
												case 'query':
													...
													var n = d.getFullYear();
													_v1=0;_v2=0;
													if(window[_k1].val()=='thisyear'){
														_v1 = n-1911;
													}else if(window[_k1].val()=='lastyear'){
														_v1 = n-1912;
													}else if($.isNumeric(window[_k1].val())){
														_v1 = parseInt($(window[_k1].selector).find('option:selected').text());
													}
													if(window[_k2].val()=='thisyear'){
														_v2 = n-1911;
													}else if(window[_k2].val()=='lastyear'){
														_v2 = n-1912;
													}else if($.isNumeric(window[_k2].val())){
														_v2 = parseInt($(window[_k2].selector).find('option:selected').text());
													}
													data = JSON.parse(StorageHandler.getTagStr(pk[0]+'_all'));
													if(_v1 && _v2){
														_v=[];_t=[];
														for(var m in data.data){
															if(data.data[m].val>=_v1 && data.data[m].val<=_v2){
														...
														for(var m in data.data){
															if(data.data[m].val==_v1){
																_r[pk[0]] = {n:json[k][pk[0]].name,class:json[k].class,type:json[k].type,v:data.data[m].t_id,t:data.data[m].val};
															}
														}
													}else if(_v2){
														for(var m in data.data){
															if(data.data[m].val==_v2){
																_r[pk[0]] = {n:json[k][pk[0]].name,class:json[k].class,type:json[k].type,v:data.data[m].t_id,t:data.data[m].val};
															}
														}
													}
...
//=============
													if(window[_k1].val() && window[_k1].val()!='-'){
														if(window[_k2].val() && window[_k2].val()!='-'){
															_r[pk[0]] = {n:json[k][pk[0]].name,class:json[k].class,type:json[k].type,v:[window[_k1].val(),window[_k2].val()],t:[$(window[_k1].selector).find('option:selected').text(),$(window[_k2].selector).find('option:selected').text()]};
														}else if(json[k][pk[0]].value[window[_k1].val()]){
															//1. _k1 is preset value(thisyear,lastyear)
															//_k1:(thisyear,lastyear)
															_r[pk[0]] = {n:json[k][pk[0]].name,class:json[k].class,type:json[k].type,v:window[_k1].val(),t:$(window[_k1].selector).find('option:selected').text()};
														}else{
															//_k1 is a number
															if(window[_k2].val() && window[_k2].val()!='-'){
																if(json[k][pk[1]].value[window[_k2].val()]){
																	//2. _k2 is preset value(thisyear,lastyear)
																	//_k1:num ~ _k2:(thisyear,lastyear)
																	//convert _k2 to num
																	var d = new Date();
																	var n = d.getFullYear();
																	switch(json[k][pk[1]].value[window[_k2].val()]){
																		case 'thisyear':
																			_k2text = n-1911;
																			break;
																		case 'lastyear':
																...
																//3. findout value between _k1~_k2
																//_k1:num ~ _k2:num
																data = JSON.parse(StorageHandler.getTagStr(pk[0]+'_all'));
																_k2text = $(window[_k2].selector).find('option:selected').text();
																_v=[];_t=[];
																for(var m in data.data){
																	if(data.data[m].val>=$(window[_k1].selector).find('option:selected').text() && data.data[m].val<=_k2text){
																		_v.push(data.data[m].t_id);
																		_t.push(data.data[m].val);
																	}
																}
...
*/
											...
											break;
										case 'of':
											if(window[pk[0]].val() && window[pk[1]].val()){
												if($(window[pk[0]].selector).find('option:selected').text()>$(window[pk[1]].selector).find('option:selected').text()){
													alert(json[k].name+'輸入的內容不正確!');
													return {};
												}
											}
										default:
	...
	function Search(_qs){
		...
		if(_pcu || _pi || _pn || _year || _ty_from || _ty_to || _py || _pty || _pcof || _pc || _prt || _pwrf){
			if(i==1) $searchpanel.find('.list>.loading').show();
