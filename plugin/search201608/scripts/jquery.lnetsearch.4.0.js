...
*/
							...
							$searchpanel.find('.year1').change(function(){
								k='year_year';
								pk = k.split('_');
								_k1=pk[0];_k2=pk[1];_sep = '/';
								if(json[k].type=='peroid'){
								...
								switch(_val){
									...
									case 'lastyear':
										//leave only thisyear
										$searchpanel.find('.'+_k2+' option').empty();
										...
										$searchpanel.find('.'+_k2).prop('disabled', false).trigger("chosen:updated");
										break;
									default:
										_text = $(this).find('option:selected').text();
										switch(_text){
											case _thisyear-1911://thisyear												//disable second select
												//leave only thisyear
												$searchpanel.find('.'+_k2+' option').empty();
												_options2 = _template_option.replace('@key@','thisyear').replace('@val@',json[k]['year'].value['thisyear']);
												$searchpanel.find('.'+_k2).append(_options2);
											...
											default:
												...
												$searchpanel.find('.'+_k2+' option').empty();
												exclude = [];
												if(_text>_thisyear-1912){
													exclude.push('lastyear');
												}
												if(_text>_thisyear-1911){
