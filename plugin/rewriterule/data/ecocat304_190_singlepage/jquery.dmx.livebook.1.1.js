...
$(document).on('pagecreate','#dmxlivebook',function(){
	...
	$('#dmxlivebook').on('scrollstop', function(e) {
		var scrollHeight = $('#paper_frame').height();
		var scrollWidth = $('#paper_frame').width();
		if(scrollHeight>scrollWidth){
			var scrollPosition = $('#dmxlivebook').height() + $('#dmxlivebook').scrollTop();
  	  // when scroll to bottom of the page
  	  if((scrollPosition > scrollHeight) || ($('#dmxlivebook').scrollTop()===0)){
    		_flip++;
    	}
    }else{
    	_flip++;
	...
	$('#dmxlivebook').mouseup(function(e){ 
		switch(e.which){
			case 1:
				$('#controllbar #l_page').trigger('vclick');
				$('#dmxlivebook').scrollTop(3);
