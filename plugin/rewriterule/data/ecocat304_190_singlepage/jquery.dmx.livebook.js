...
(function($) {
    ...
    var DEF_PARAMS = {
        ...
        'WEBCRM': {
            'URL': 'https://ebook.ctdp.org.tw/plugin/bigdata/api/webcrm_rec.php',
            'METHOD': 'get'
        },
        'THRESHOLD': { //イベント開始の閾値
            'ZOOMIN_SCALE': 1.3,
    ...
    var params = {
        ...
        'basezlv': 5, //非拡大時に使用する拡大率（zoomlvsの配列インデックス）
	...
	//Anderson 20181009 (add _status.scalingratio)
        var x = parseInt(x1) * ps * params.basescale * _status.scalingratio;
        var y = parseInt(y1) * ps * params.basescale * _status.scalingratio;

        var w = (parseInt(x2) - otbw) * ps * params.basescale * _status.scalingratio;
        var h = (parseInt(y2) - otbw) * ps * params.basescale * _status.scalingratio;
	...
	//Anderson 20181009 (Add _status.scalingratio)	
        var x = parseInt(x1) / params.searchall.datapntratio * ps * params.basescale * _status.scalingratio;
        var y = parseInt(y1) / params.searchall.datapntratio * ps * params.basescale * _status.scalingratio;

        var w = (parseInt(x2) - parseInt(x1)) / params.searchall.datapntratio * ps * params.basescale * _status.scalingratio;
        var h = (parseInt(y2) - parseInt(y1)) / params.searchall.datapntratio * ps * params.basescale * _status.scalingratio;
        $(this).css('left', x+'px').css('top', y+'px').css('background-color', highlightColors[ci] )
               .width(w).height(h);
    };
	...
	if(pph >= ppw && !_ismobile){
        	//Anderson modify 20181101
		//_status.scalingratio = (win_w-300)/win_h * 1.5;
		_status.scalingratio = win_w/win_h*_status.rate;
        }else{
                _status.scalingratio = 1;
	}
	//Anderson
	if( _ismobile ) {
        	ppx = ppo.l + parseInt(params.paper_ox * posw + (ppo.w - ppw) / 2);
        }else{
        	ppx = ppo.l + parseInt(params.paper_ox * posw * _status.scalingratio + (ppo.w - ppw * _status.scalingratio) / 2);
        }
...
*/
        return {'wsw': wsw, 'wsh': wsh,
                ...
                'ppx': ppx, 'ppy': ppy,
                'ppw': ppw * _status.scalingratio, 'pph': pph * _status.scalingratio,
                'mdw': mdw * _status.scalingratio, 'mdh': mdh * _status.scalingratio};
    };
        ...
        if (params.usewebcrm) {
            $.ajax({ 
                ...
                data: {
                		...
                		'uid_type': params.options.uidtype,
    ...
    var _orientationChange = function (o, t, f) {
		...
		//Anderson 20200305
		var measureStringHW = function(arg){
			if(!$('#work').length) $('body').append('<div id="work" style="font-size:12px;display:inline-block;"></div>');
			let l = [], ch=12, cw=6, fs=12, lh=32;
			let arr = (Array.isArray(arg))?arg:[arg];
			for(var i=0;i<arr.length;i++){
				//$('#work').text(arr[i]);
				let test = document.getElementById("work");
				test.textContent = arr[i];
				l.push({txt:arr[i],x1:5,y1:100+lh*i,x2:5+test.clientWidth+1,y2:100+lh*i+test.clientHeight+1,h:test.clientHeight + 1,w:test.clientWidth + 1});
				//console.log(arr[i]+': h='+(test.clientHeight + 1)+'px ;w='+(test.clientWidth + 1)+'px');
			}
			return l;
		};
		var allIndexOf = function(str, toSearch) {
		  let indices = [];
		  let keyreg = new RegExp(toSearch, "i");
		  for(var pos = str.regexIndexOf(keyreg,0); pos !== -1; pos = str.regexIndexOf(keyreg, pos + 1)) {
				indices.push(pos);
		  }
		  return indices;
		};
...
//                    methods._initError('[Err:00001-005] タッチイベントがサポートされていないデバイスです。', DEF_PARAMS.ERROR.FATAL);
            ...
            if (this.length === 0) {
                ...
                if (status.initready) {
                    ...
                    methods.resize.apply($this, [data.target,true]);
                    $(window).bind("resize", {'thisobj': $this}, methods.resizeHandler);
		    //$(window).bind("resize", {'thisobj': $this}, $.debounce(250,true,function() { methods.resizeHandler}) );
                    ...
                    _ismobile = ismobile();
                    if(!_ismobile){
	                    _status.scalingratio = $(window).innerWidth()/$(window).innerHeight() * _status.rate;
		    }
                    ...
                    if(settings.keys){
                    	$('#search_all').click();
                    	$('#search_keyword').val(settings.keys.replace(/\,/g,' '));
                    	$('#search_btn').click();
                    }
                }
            }
            ...
            $.ajax({ 
                ...
                success: function(xml, status){
                    if(status === 'success') {
                        ...
                        //Anderson 20190411, set book key to webcrm_medianame
                        if(params.webcrm_medianame==''){
                        	_s = document.location.href.split('/ebook/');
                        	//_s = document.location.href.split('/files/');
                        	_arr = _s[1].split('/');
                        	params.webcrm_medianame=_arr[0];
                        }
...
//            methods.createzoommovebutton.apply($this, [data.target]);
            ...
            $(data.target).on('mousedown', selectors.linkrect, function(e){
                $(e.target).css('opacity', selectors.linkrect.opacityover); //リンクの透過率を変更（アクティブ表示）
                    ...
                    if (t == DEF_PARAMS.LINKTYPE.PAGE) {
                        if (status.spreadmode === DEF_PARAMS.SPREADMODE.DOUBLE) { //見開きの場合
                          ...
                          status.sliding = true;
	                        ...
	                        methods.gotostart.apply($this, [data.target, $(e.target).data('link'), cue]);
                        }
                    		...
                    		//window.open($(e.target).data('link'), $(e.target).data('target'));
												...
												////// 以下os為ios或android時可以呼叫app method
												if (os == "android" || os == "ios") {
													i = document.location.href.lastIndexOf('/html5/');
													basepath = document.location.href.substring(0,i);alert(basepath+'/'+$(e.target).data('link'));                                                                                                        ExtCall.popWebviewBox('', basepath+'/'+$(e.target).data('link'),  '');

													ExtCall.popWebviewBox('', basepath+'/'+$(e.target).data('link'),  '');
												}else{
													window.open($(e.target).data('link'), $(e.target).data('target'));
												}
                    }
            ...
            indexlist_gotopage_fnc = function(e){
                if (status.flipping === false && status.sliding === false && status.changespread === false) {
                    ...
                    if (status.searchall) {
			methods.createsearchrect.apply($(this), [data.target, $(e.currentTarget).data('link')]);
...
//                    methods.resetsearchallposition.apply($this, [$this]);	//@ToDo
            ...
            $(document).on("vclick", '#search_btn', function(e){
        		if (params.searchall.dataloaded) {
                    ...
                    //Anderson 20200306
                    searchMatchKeywordIndexs = new Array();
                    searchMatchTxtIndexs = new Array();
                    var _matchk = new Array();
                    var _matcht = new Array();
                    ...
                    keyword = keyword.replace(/( |　)+/g, " ");
                    //Anderson 20200306
                    for(var i=0;i<regexSpecialCharacters.length;i++){
                    	keyword = keyword.replace(new RegExp("\\"+regexSpecialCharacters[i],"g"),"\\"+regexSpecialCharacters[i]);
                    }
                    keys = keyword.split(" ");
                    ...
                    var coords = params.searchall.search_pnt;
										...
										//Anderson 20200305 Add
										var stringsHW = measureStringHW(strings);
										kml = keys.reduce(function (result, val) {
									      return Math.max(result, !isNaN(val) ? parseInt(val) : val.length);
									  }, 0);
									  if($.isEmptyObject(params.searchall.wrepStrings)){
										  var wrepStrings = [];
										  if(kml>1 && strings.length>1){
												for(var i=1;i<strings.length;i++){
													wrepStrings.push(strings[i-1]+strings[i].substr(0,kml));
												}
										  }
										  params.searchall.wrepStrings = wrepStrings;
										}else{
											wrepStrings = params.searchall.wrepStrings;
										}

                    for (var i = 0, l = wrepStrings.length; i < l; i++) {
                        var is_hit = false,is_hit1 = false, hasKeyword=false, keywordOnWrep=false;
                        //Anderson 20200306
                        var coord = String(coords[i]).split(";");
                        var nextCoord = String(coords[i+1]).split(";");
                        coord = coord.map(function(x){return parseInt(x);});
                        nextCoord = nextCoord.map(function(x){return parseInt(x);});
                        stringsHW[i]['r'] = (coord[2]-coord[0])/stringsHW[i]['w'];
                        stringsHW[i+1]['r'] = (nextCoord[2]-nextCoord[0])/stringsHW[i+1]['w'];
												txt2 = strings[i];
												txt3 = wrepStrings[i];
                        for (var j = 0, m = keys.length; j < m; j++) {
                        		/*Anderson 20200306
                            var keyreg = new RegExp(keys[j], "i");
                            var p = String(wrepStrings[i].substr(kml/2-keys[k].length,keys[k].length*2)).search(keyreg);
                            if (0 <= p) {
                            */
			    var _fr = false;
                            _ci = j % (bookmarkColors.length/2);
                            var str = wrepStrings[i].substr(0,strings[i].length+keys[j].length-1);
														var pos = allIndexOf(str,keys[j]);
														if (pos.length>0) {
																hasKeyword=true;
																for(var k=0;k<pos.length;k++){
																	_HitNums = 0;
																	if(strings[i].length-pos[k]<keys[j].length){
																		keywordOnWrep=true;
																		wl1 = pos[k]-strings[i].length;
																		wl2 = keys[j].length+wl1;
																		ss = strings[i].substr(wl1);
																		r1 = measureStringHW(ss);
																		se = strings[i+1].substr(0,wl2);
																		r2 = measureStringHW(se);
																		np = parseInt(coord[4])+1;
																		r = str.substr(pos[k],keys[j].length);
																		txt1 = strings[i].replaceAll(r,'['+r+']');
																		//txt3 = txt3.replace(r,'<span style="color:red">'+r.replace(ss,ss+' \\n ')+'</span>');
				//_arr = txt3.split(r);
				//_HitNums = _arr.length-1;
				  _HitNums = (txt3.match(new RegExp(r,"gi")) || []).length;
				if(!_fr){
																		txt3 = txt3.replaceAll(r,'<span style="opacity: 0.7;color: #000;background-color:'+highlightColors[_ci]+'">'+r+'</span>');
					_fr = true;
				}
																		if (!$.isArray(searchMatchKeywordIndexs['page_' + coord[4]]))
																			searchMatchKeywordIndexs['page_' + coord[4]] = new Array();
																		searchMatchKeywordIndexs['page_' + coord[4]].push({
																			"txt":txt1,
																			"page":coord[4],
																			"ci":_ci,
																			"x1":parseInt(coord[0])+(stringsHW[i].w-r1[0].w)*stringsHW[i].r+xs,
																			"y1":parseInt(coord[1])+ys,
																			"x2":parseInt(coord[0])+xs+xa,
																			"y2":parseInt(coord[3])+ys+ya});
																		if (!$.isArray(searchMatchKeywordIndexs['page_' + nextCoord[4]]))
																			searchMatchKeywordIndexs['page_' + nextCoord[4]] = new Array();
																		searchMatchKeywordIndexs['page_' + nextCoord[4]].push({
																			"txt":txt1,
																			"page":nextCoord[4],
																			"ci":_ci,
																			"x1":parseInt(nextCoord[0])+xs,
																			"y1":parseInt(nextCoord[1])+ys,
																			"x2":parseInt(nextCoord[0])+r2[0].w*stringsHW[i+1].r+xs+xa,
																			"y2":parseInt(nextCoord[3])+ys+ya});
																	}else{
																		ss = strings[i].substr(0,pos[k]);
																		r1 = measureStringHW(ss);
																		se = strings[i].substr(0,pos[k])+keys[j];
																		r2 = measureStringHW(se);
																		r = str.substr(pos[k],keys[j].length);
																		txt1 = strings[i].replaceAll(r,'['+r+']');
				//_arr = txt2.split(r);
				//_HitNums = _arr.length-1;
				  _HitNums = (txt3.match(new RegExp(r,"gi")) || []).length;
				if(!_fr){
					txt2 = txt2.replaceAll(r,'<span style="opacity: 0.7;color: #000;background-color:'+highlightColors[_ci]+'">'+r+'</span>');
					_fr = true;
				}
																		if (!$.isArray(searchMatchKeywordIndexs['page_' + coord[4]]))
																			searchMatchKeywordIndexs['page_' + coord[4]] = new Array();
																		searchMatchKeywordIndexs['page_' + coord[4]].push({
																			...
																			"page":coord[4],
																			"ci":_ci,
																			"x1":parseInt(coord[0])+r1[0].w*stringsHW[i].r+xs,
																			"y1":parseInt(coord[1])+ys,
																			"x2":parseInt(coord[0])+r2[0].w*stringsHW[i].r+xs+xa,
																			"y2":parseInt(coord[3])+ys+ya});
																	}
																}
                                ...
                                searchMatchPageHitNums['page_' + coord[4]]+=_HitNums;
                                ...
                                if (!is_hit) {
                                    is_hit = true;
                                    //Anderson 20200307 deprecate
                                    //searchMatchPageIndexes['page_' + coord[4]].push({"txt": strings[i], "page": coord[4], "x1": coord[0], "y1": coord[1], "x2": coord[2], "y2": coord[3]});
                                }
                            }
                        }
                        //Anderson 20200306
                        if(hasKeyword && !is_hit1){
                        	is_hit1 = true;
													if (!$.isArray(searchMatchTxtIndexs['page_' + coord[4]]))
														searchMatchTxtIndexs['page_' + coord[4]] = new Array();
                        	if(keywordOnWrep){
                        		searchMatchTxtIndexs['page_' + coord[4]].push({"txt":txt3,"page":coord[4]});
                        	}else{
														searchMatchTxtIndexs['page_' + coord[4]].push({"txt":txt2,"page":coord[4]});
													}
												}
                    }
                        ...
                        for (var it in searchMatchPageHitKeys) {
                            for (var i = 0, l = keys.length; i < l; i++) {
                                if (searchMatchPageHitKeys[it][i] == undefined) {
                                    ...
                                    //Anderson 20200306
                                    delete searchMatchKeywordIndexs[it];
                                    delete searchMatchTxtIndexs[it];
                                    break;
                                }
                            }
                        }
...
*/
										//Anderson 20200306
										for (var it in searchMatchKeywordIndexs) {
												_matchk = _matchk.concat(searchMatchKeywordIndexs[it]);
										}
										for (var it in searchMatchTxtIndexs) {
												_matcht = _matcht.concat(searchMatchTxtIndexs[it]);
										}
										searchMatchIndexes = [];
                    for (var i = 0, l = _matcht.length; i < l; i++) {
                        if (!$.isArray(searchMatchIndexes['page_' + _matcht[i]['page']]))
                            searchMatchIndexes['page_' + _matcht[i]['page']] = new Array();
                        searchMatchIndexes['page_' + _matcht[i]['page']].push(i);
                    }
                    
										params.searchall.matchk = _matchk;
										_match = _matchk;

										params.searchall.matcht = _matcht;
                    ...
                    if (!resultFlg) {
                        ...
                        methods.createsearchresult.apply($this, [data.target, _matcht, params.searchall.sort_mode]);
                    }
                } else {
            ...
            $(document).on("vclick", '#search_sortorder_hit', function(e){
            	if (params.searchall.sort_mode == "hitnum") {
                	...
                	methods.createsearchresult.apply($this, [data.target, params.searchall.matcht, params.searchall.sort_mode]);
                }
            });
        ...
        adjustnombre: function (ps){
        	...
        	$(selectors.pagenombre).css({'font-size': (settings.disp_nombre_fontsize * params.basescale * ps * 2) + 'px'});
		...
		//20180906 Anderson
        	...
        	$(selectors.pagenombrel).css({'right': (settings.disp_nombre_posside * params.basescale * ps) + 'px'});
        ...
        createsearchresult: function (){
            ...
            if (_match.length >= 1) {
            		_r = 0;
								for(var hn in searchMatchPageHitNums){
									_r += searchMatchPageHitNums[hn];
								}
                ...
                $("#search_result_info span").text(format_lang(DEF_PARAMS.LANG.SEARCHALL.HIT, _r));
                // sort : page number
                if (_sortmode == "page") {
        ...
        adjustbookmarktag: function(){
            ...
            for (var i = 0; i < bookmarkObj.tag.length; i++) {
                ...
                if (page_num_b < cur_page_num_b || cur_page_num_b + adjust_page < page_num_b) {
        ...
        zoominstart: function (){
            ...
            //Anderson modify 20180518
	    _ismobile = ismobile();
	    _rate=(_ismobile)?1:_status.scalingratio;
	    ...
	    _rate=(_ismobile)?1:_status.scalingratio;
            ...
            if (_zoom_w < params.zoom_outer_w) {
            	zx = (params.zoom_outer_w - _zoom_w) / 2;
            	//zx = 0;
            } else {
	            if (0 < zx) zx = 0;
            ...
            if (_zoom_h < params.zoom_outer_h) {
            	zy = (params.zoom_outer_h - _zoom_h) / 2;
            } else {
	            if (0 < zy) zy = 0;
	            else if (zy < params.zoom_outer_h - _zoom_h) zy = params.zoom_outer_h - _zoom_h;
            }
        ...
        zoomdragterminate: function () {
            ...
            _ismobile = ismobile();
            _rate=(_ismobile)?1:_status.scalingratio;
            _zoom_w = params.zoom_pw*_rate;
            _zoom_h = params.zoom_ph*_rate;
...
//        	cue.timerid = setTimeout(methods.slideaction, params.slidedelay, $(this), [t, DEF_PARAMS.SLIDEMODE.SLIDE, cue.clp, cue.crp, cue.nlp, cue.nrp, cue]);
		...
		//Anderson 20181013 (add _status.scalingratio)                 
                var x = parseInt(x1) * params.pscale * params.basescale * _status.scalingratio;
                var y = parseInt(y1) * params.pscale * params.basescale * _status.scalingratio;
                
                var w = (parseInt(x2) - otbw) * params.pscale * params.basescale * _status.scalingratio;
                var h = (parseInt(y2) - otbw) * params.pscale * params.basescale * _status.scalingratio;
        ...
        createlinkrectzoomarea: function(){
            ...
            $(".page[rel=" + p + "] .link_rect", selectors.paper_frame).each(function(){
                ...
                var x = parseInt(x1) * params.zscale.scale * _status.scalingratio;
                var y = parseInt(y1) * params.zscale.scale * _status.scalingratio;
                
                var w = (parseInt(x2) - otbw) * params.zscale.scale * _status.scalingratio;
                var h = (parseInt(y2) - otbw) * params.zscale.scale * _status.scalingratio;
        ...
        createsearchrect: function(){
            ...
            var cleared = false;
						...
						//Anderson 20200306
						$(".page div.search_rect", selectors.paper_frame).remove();
            ...
            for (var i = 0, l = _match.length; i < l; i++) {
                ...
                //Anderson 20181009 (Add _status.scalingratio)
                var x = parseInt(x1) / params.searchall.datapntratio * params.pscale * params.basescale * _status.scalingratio;
                var y = parseInt(y1) / params.searchall.datapntratio * params.pscale * params.basescale * _status.scalingratio;

                var w = (parseInt(x2) - parseInt(x1)) / params.searchall.datapntratio * params.pscale * params.basescale * _status.scalingratio;
                var h = (parseInt(y2) - parseInt(y1)) / params.searchall.datapntratio * params.pscale * params.basescale * _status.scalingratio;
//console.log('x='+x+';y='+y+';w='+w+';h='+h+';txt='+_match[i]["txt"]);
//console.log('params.searchall.datapntratio='+params.searchall.datapntratio+';params.pscale='+params.pscale+';params.basescale='+params.basescale+';_status.scalingratio='+_status.scalingratio);

                if (tp == p) { //show search one page per result
                	if (cleared === false) {
                		cleared = true;
                	...
                	//Anderson 20200805
	                html = '<div class="search_rect" style="background-color:' + highlightColors[ci] + '; top:' + y + 'px; left:' + x + 'px; width:' + w + 'px; height:' + h + 'px;"'
            			+ ' data-pos="'+x1+','+y1+','+x2+','+y2+'" data-ci="'+ci+'"></div>';
...
/*
		            $(".page[rel=" + p + "] .search_rect:eq("+i+")", selectors.paper_frame).css({
		                ...
		                'border-color': '#F00',
		                'border-width': '1px',
		                'background-color': highlightColors[ci]
		            });
*/
            }
        },
        createsearchrectzoomarea: function(){
            ...
            for (var i = 0, l = _match.length; i < l; i++) {
                ...
                if ($("#zoom_frame div.page[rel=" + p + "]").length) {
                    ...
                    var x = parseInt(x1) / params.searchall.datapntratio * params.zscale.scale * _status.scalingratio;
                    var y = parseInt(y1) / params.searchall.datapntratio * params.zscale.scale * _status.scalingratio;

                    var w = (parseInt(x2) - parseInt(x1)) / params.searchall.datapntratio * params.zscale.scale * _status.scalingratio;
                    var h = (parseInt(y2) - parseInt(y1)) / params.searchall.datapntratio * params.zscale.scale * _status.scalingratio;

                		//Anderson 20200512
                    html = '<div class="search_rect" style="top:' + y + 'px; left:' + x + 'px; width:' + w + 'px; height:' + h + 'px; background-color:'+ highlightColors[ci] +'"'
                    + 'data-pos="'+x1+','+y1+','+x2+','+y2+'" data-ci="'+ci+'"></div>';
        ...
        resize: function(){
            ...
            var $this = arguments[0],
                data = $this.data('dmxLivebook');
	    var do_search = typeof arguments[1] === 'undefined' ? false:arguments[1];
		...
		if(do_search){
			...
			//methods.createsearchresult.apply($this, [data.target, params.searchall.matcht, params.searchall.sort_mode]);
			methods.createsearchrect.apply($(this), [data.target, status.curbasepage]);
		}
...
$(document).on('pagecreate','#dmxlivebook',function(){
	...
	$('body').keyup(function(e){
		if(!$('#goto_text').is(':focus') && $('#searchall').css('display')=='none' && $('#vindex').css('display')=='none'){
		switch(e.which){
			case 37:
				$('#controllbar #l_page').trigger('vclick');
