...
var selector_header_banner = '#body>#header>.banner';
...
/*
compare cate of api and localstorage
remove books in subcate which is diff from API
show books, if in localstorage than use it, if not in localstorage than get from API
*/
var StorageHandler = {
	pagesize:15,
	bsk:'bslist',
	ck:'bsm',
	bk:'bsbook',
	SourceMode:['all','new','my'],
	lskey:[],
	_lskey:[],
	_is_complete:false,
	makeBSListKey : function(_isExpiredList,_uid,_buid){
		_str = this.bsk;
	...
	makeCateListKey : function(bsid){
		//return this.ck+'_'+bsid+'_'+mcid+'_'+scid;
		return this.ck+'_'+bsid;
	},
	makeBookListKey : function(_mode,scid,i){
		return this.bk+'_'+_mode+'_'+scid+'_'+i;
	},
	getBSListStr : function(_isExpiredList,_uid,_buid){
		_key = this.makeBSListKey(_isExpiredList,_uid,_buid);
		return window.localStorage[_key];
	},
	setBSListStr : function(_isExpiredList,_uid,_buid,str){
		_key = this.makeBSListKey(_isExpiredList,_uid,_buid);
		window.localStorage[_key] = str;
	},
	getCateStr : function(bsid){
		_key = this.makeCateListKey(bsid);
		return window.localStorage[_key];
	},
	setCateStr : function(bsid,str){
		_key = this.makeCateListKey(bsid);
		window.localStorage[_key] = str;
	},
	getBookStr : function(_mode,scid,i){
		_key = this.makeBookListKey(_mode,scid,i);
		return window.localStorage[_key];
	},
	setBookStr : function(_mode,scid,i,str){
		_key = this.makeBookListKey(_mode,scid,i);
		if(typeof str !== 'string') str = JSON.stringify(str);
		window.localStorage[_key] = str;
	},
	getTagStr : function(tpkey){
		return window.localStorage['ttiitag_'+tpkey];
	},
	setTagStr : function(tpkey,str){
		if(typeof str !== 'string') str = JSON.stringify(str);
		window.localStorage['ttiitag_'+tpkey] = str;
	},
	getSearchItemSettingStr : function(uid){
		if(uid){
			return window.localStorage['ttiisearchsetting'+uid];
		}
	},
	setSearchItemSettingStr : function(uid,str){
		if(uid){
			if(typeof str !== 'string') str = JSON.stringify(str);
			window.localStorage['ttiisearchsetting'+uid] = str;
		}
	},
	getQuickSearchStr : function(uid){
		if(uid){
			if(window.localStorage['ttiiquicksearch'+uid]){
				return window.localStorage['ttiiquicksearch'+uid];
			}else{
				return '[]';
			}
		}
	},
	setQuickSearchStr : function(uid,str){
		if(uid){
			if(typeof str !== 'string') str = JSON.stringify(str);
			window.localStorage['ttiiquicksearch'+uid] = str;
		}
	},
	hasLocalStorage : function(){
		return (window.localStorage);
	},
	_doLocalStorageKey : function(_key){
		_arr = _key.split('_');
		if(_arr.length!=4) return;

		switch(_arr[0]){
			//case this.ck:
			case this.bk:
				if(!(_arr[0] in this.lskey)){
					this.lskey[_arr[0]] = [];
				}
				if(!(_arr[1] in this.lskey[_arr[0]])){
					this.lskey[_arr[0]][_arr[1]] = [];
				}
				if(!(_arr[2] in this.lskey[_arr[0]][_arr[1]])){
					this.lskey[_arr[0]][_arr[1]][_arr[2]] = [];
				}
				this.lskey[_arr[0]][_arr[1]][_arr[2]].push(_arr[3]);
				break;
		}
	},
	doLocalStorageKey : function(bsid){
		this.lskey = [];
		this.lskey[this.bk] = [];
		this.lskey[this.ck] = [];
		for(sc in this.SourceMode){this.lskey[this.bk][sc]=[];}
		this.lskey[this.ck][bsid] = [];
		for(i=0;i<window.localStorage.length;i++){
			_str = window.localStorage.key(i);
			this._doLocalStorageKey(_str);
		}
	},
	doCate : function(bsid){
		this._lskey[this.ck]=[];
		this._lskey[this.ck][bsid]=[];
		APIHandler.getMenuJSON(function(catejson){
			for(i=0;i<catejson.length;i++){
				StorageHandler._lskey[StorageHandler.ck][bsid][catejson[i].c_id]=[];
				if(catejson[i].sub_category){
					for(j=0;j<catejson[i].sub_category.length;j++){
						StorageHandler._lskey[StorageHandler.ck][bsid][catejson[i].c_id].push(catejson[i].sub_category[j].c_id);
					}
				}
			}
		});
	...
	pageIndex:[],
	getLoadingIndex : function(scid){
		if(this.pageIndex[scid]){
			return this.pageIndex[scid];
		}else{
			this.pageIndex[scid]=1;
			return this.pageIndex[scid];
		}
	},
	nextLoadingIndex : function(scid){
		if(this.pageIndex[scid]){
			++this.pageIndex[scid];
			return this.pageIndex[scid];
		}else{
	...
	resetLoadingIndex : function(scid){
		this.pageIndex[scid]=0;
	},
	init : function(bsid){
		this.doLocalStorageKey(bsid);
		this.doCate(bsid);
		//compare localStorage and Server category of bookshelf
		//newest cate list is _lskey
		//newest book list is lskey
		if(Object.keys(this.lskey[this.ck][bsid]).join()!=
			Object.keys(this._lskey[this.ck][bsid]).join()){
			//remove all
			for(mc in this.lskey[this.ck][bsid]){
				//if LocalStorage main cate is not in API
				if(this._lskey[this.ck][bsid].indexOf(mc)==-1){
					for(sc in this.lskey[this.ck][bsid][mc]){
						//remove localstorage cate
						_key = this.makeCateListKey(bsid,mc,sc);
						window.localStorage.removeItem(_key);
						for(sm in this.lskey[this.bk]){
							for(i in this.lskey[this.bk][sm][sc]){
								//remove localstorage book
								_key = this.makeBookListKey(sm,sc,i);
								window.localStorage.removeItem(_key);
							}
						}
					}
	...
	_sc:true,
	_arrsubcate:false,
	getNextCateObj : function(){
		if(this._sc===true){
			if(this._arrsubcate.length==0) return -1;
			this._sc = this._arrsubcate[0];
			return this._sc;
		}else{
			//i = this._arrsubcate.map(x=>x.c_id).indexOf(this._sc.c_id);
			i = this._arrsubcate.map(function(element,index,array){return element.c_id}).indexOf(this._sc.c_id);
			if((i+1)>=this._arrsubcate.length){
				return [];
			}else{
				this._sc = this._arrsubcate[i+1];
				return this._sc;
			}
		}
	},
	resetCateObjIndex : function(){
		this._sc=true;
	},
	processCate : function(cid){
		this.mcobj=[];
		this.scobj=[];
		//maincate?
		if(!$.isEmptyObject(bookEnv.MenuJSON)){
			this.mcobj = bookEnv.MenuJSON.filter(function(cate){
				return cate.c_id==cid;
			});
		}
		if(!$.isEmptyObject(bookEnv.MenuJSON)){
			//subcate?
			$.each(bookEnv.MenuJSON,function(i,v){
				if(!$.isEmptyObject(v.sub_category)){
					_scobj = v.sub_category.filter(function(cate){
						return cate.c_id==cid;
					});
					StorageHandler.scobj = StorageHandler.scobj.concat(_scobj);
				}
			});
		}
		if($.isEmptyObject(this.mcobj) && $.isEmptyObject(this.scobj)){
			return null;
		}
		_fm = false;
		_fs = false;
		if(!$.isEmptyObject(this.mcobj)){
			if(this.mcobj[0].sub_category){
				this._arrsubcate = this.mcobj[0].sub_category.slice();
			}else{
				_fm = true;
			}
		}else{
		...
		if(!$.isEmptyObject(this.scobj)){
			this._arrsubcate = this.scobj.slice();
		}else{
			_fs = true;
		}
		if(_fm && _fs){
		...
		return this._arrsubcate;
	},
	_dynamicLoadBooks : function(cid,i,returnFunction){
		//count how many book in bookshelf
		//check how many book load from api
		//keep load book packages until EOF
		...
		APIHandler.getBookPackageJSON(_mode,cid,i,function(data){
				if($.isEmptyObject(data)){
					StorageHandler.resetLoadingIndex(cid);
				}
				returnFunction(data);
		});
	},
	getNext : function(cid,i,returnFunction){
		setTimeout(function(){
			StorageHandler._dynamicLoadBooks(cid,i,function(data){
				returnFunction(data);
			});
		},0);
	}
...
var StorageAction = function(){
		var _msgHandler = new MsgAction('StorageAction');
		Init();
		function Init(){
			StorageHandler.init(bs_id);
		}
}
...
var APIHandler ={
    ...
    validBookshelfList : function(_isExpiredList,_buid,_uid,returnFunction){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(!StorageHandler.getBSListStr(_isExpiredList,_uid,_buid)){
                	returnFunction(false);
                }else{
                	returnFunction(ToolsHandler.MD5(lzw_decode(StorageHandler.getBSListStr(_isExpiredList,_uid,_buid)))==data);
                }
            },
            error: function(){
    ...
    getBookshelfList : function(_isExpiredList,_buid,_uid,returnFunction){
        ...
        APIHandler.validBookshelfList(_isExpiredList,_buid,_uid,function(v){
        	if(v){
        		_val=$.parseJSON(lzw_decode(StorageHandler.getBSListStr(_isExpiredList,_uid,_buid)));
        		returnFunction(_val);
        	}else{
		        $.ajax({
		            type: "post",
		            ...
		            success: function(data){
		            		...
		            		StorageHandler.setBSListStr(_isExpiredList,_uid,_buid,_val);
		                _msgHadler.Log('getBookshelfList data='+_val);
    ...
    validMenuJSON : function(returnFunction){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(!StorageHandler.getCateStr(bs_id)){
                	returnFunction(false);
                }else{
                	returnFunction(ToolsHandler.MD5(lzw_decode(StorageHandler.getCateStr(bs_id)))==data);
                }
            },
            error: function(){
    ...
    getMenuJSON : function(returnFunction){
        var _msgHadler = new MsgAction('APIHandler');
        APIHandler.validMenuJSON(function(v){
        	if(v){
        		bookEnv.MenuJSON=$.parseJSON(lzw_decode(StorageHandler.getCateStr(bs_id)));
        		if(returnFunction) returnFunction(bookEnv.MenuJSON);
        	}else{
		        $.ajax({
		            type: "post",
		            dataType: "json",
		            ...
		            success: function(data){
		                ...
		                StorageHandler.setCateStr(bs_id,JSON.stringify(data));
    ...
    _getBookPackageParams : function(_mode,_cid,i){
			var params = {index:i,bs:bs_id};
			switch(_mode){
				case 'new':
					$.extend(params,{cid:0,u_id:uid});
					break;
				case 'all':
					$.extend(params,{cid:0,u_id:uid});
					break;
				case 'my':
					$.extend(params,{buid:-1});
					break;
				default:
					$.extend(params,{cid:_cid,u_id:uid});
					break;
			}
    	return params;
    },
    validBookPackageJSON : function(_mode,_cid,i,returnFunction){
        var _msgHadler = new MsgAction('APIHandler');
        var params = this._getBookPackageParams(_mode,_cid,i);
        $.extend(params,{cmd:'valid'});
        $.ajax({
            type: "post",
            dataType: "text",
            ...
            data: params,
            success: function(data){
	              _msgHadler.Log('validBookPackageJSON data='+data);
	              _msgHadler.Log('Reveive validBookPackageJSON success!');
								if(_cid!=null){
									_str = StorageHandler.getBookStr(_mode,_cid,i);
									if(_str){
										returnFunction(ToolsHandler.MD5(lzw_decode(_str))==data);
									}else{
										returnFunction(false);
									}
								}
            ...
            error: function(){
                _msgHadler.Log('Reveive validBookPackageJSON fail!');
            },
            async:false
        });
    },
    getBookPackageJSON : function(_mode,_cid,i,returnfunction){
        var _msgHadler = new MsgAction('APIHandler');
        var params = this._getBookPackageParams(_mode,_cid,i);
        APIHandler.validBookPackageJSON(_mode,_cid,i,function(v){
        	if(v){
        		if(_cid!=null){
        			_str = StorageHandler.getBookStr(_mode,_cid,i);
        			if(_str){
        				bookEnv.BooksByCateJSON=$.parseJSON(lzw_decode(_str));
        				returnfunction(bookEnv.BooksByCateJSON);
        			}
        		}
        	}else{
		        $.ajax({
		            ...
		            success: function(data){
	                  ...
	                  _msgHadler.Log('getBookPackageJSON data='+_str);
	                  if(!$.isEmptyObject(data)){
	  	                StorageHandler.setBookStr(_mode,_cid,i,_str);
	  	              }
	                  returnfunction(data);
	  	            }
			            _msgHadler.Log('Reveive getBookPackageJSON success!');
			          },
		            error: function(){
		                bookEnv.IsDefaultData=true;
		                bookEnv.BooksJSON=data_allbook;
		                bookEnv.BooksByCateJSON=data_allbook;
		                _msgHadler.Log('getBookPackageJSON data='+JSON.stringify(data_allbook));
		                _msgHadler.Log('Reveive getBookPackageJSON fail!');
		            },
		            async:false
		        });
		      }
        ...
        APIHandler.validBooksJSON(_cid,function(v){
        	if(v){
        		if(_cid==null){
        			...
        			bookEnv.BooksByCateJSON=$.parseJSON(lzw_decode(window.localStorage['bspb'+_cid+'_'+_pager_bid]));
        		}
        	}else{
		        $.ajax({
		            type: "post",
    ...
    getMainCateBooksJSON : function(_cid,_pager_bid){
        var _msgHadler = new MsgAction('APIHandler');
        $.ajax({
            type: "post",
            dataType: "json",
    ...
    getMyBooksJSON : function(_cid,_pager_bid){
        var _msgHadler = new MsgAction('APIHandler');
        $.ajax({
            type: "post",
            dataType: "json",
    ...
    loginCheck : function(returnFunction){
        ...
        $.ajax({
            ...
            data: {},
            success: function(data){
                _msgHadler.Log('loginCheck data='+JSON.stringify(data));
                _msgHadler.Log('Send loginCheck success!');
                returnFunction(data);
            },
            error: function(){
    ...
    loginCheck1 : function(_url,returnFunction){
        var _msgHadler = new MsgAction('APIHandler');
        $.ajax({
            type: "post",
            dataType: "text",
            url: _basepath+"/api/login_check1.php?"+Date.now(),
	    data: {u:_url},
            success: function(data){
                _msgHadler.Log('loginCheck1 data='+JSON.stringify(data));
                _msgHadler.Log('Send loginCheck1 success!');
                returnFunction(data);
            },
            error: function(){
                _msgHadler.Log('Send loginCheck1 fail!');
                return false;
            },
            async:false
        });
    },
    buinfo : function(returnFunction){
        var _msgHadler = new MsgAction('APIHandler');
        $.ajax({
            type: "post",
            dataType: "json",
            ...
            data: {cmd:'group'},
            success: function(data){
                _msgHadler.Log('buinfo data='+JSON.stringify(data));
                _msgHadler.Log('Send buinfo success!');
                returnFunction(data);
            },
            error: function(){
                _msgHadler.Log('Send buinfo fail!');
                return false;
            },
            async:false
        });
    ...
    forget : function(_acc,_email,returnFunction){
        var _msgHadler = new MsgAction('APIHandler');
        $.ajax({
            type: "post",
            dataType: "json",
            url: _basepath+"/api/api.php?cmd=forget&"+Date.now(),
            data: {acc:_acc, email:_email},
            success: function(data){
                _msgHadler.Log('Send forget success!');
                returnFunction(data);
            },
            error: function(){
                _msgHadler.Log('Send forget fail!');
            }
        });
    },
    ...
    setLang : function(_lang,returnFunction){
        var _msgHadler = new MsgAction('APIHandler');
        $.ajax({
            type: "post",
            url: _basepath+"/api/api.php?cmd=setLang&"+Date.now(),
            data: {lang:_lang},
	    dataType: 'html',
            success: function(data){
                _msgHadler.Log('Send setLang success!');
                returnFunction(data);
                return true;
            },
            error: function(){
                _msgHadler.Log('Send setLang fail!');
                return false;
            },
            async:false
        });
            ...
            if(document.location.href.indexOf(systemEnv.devicePath)==-1){
               ...
               return;
				...
				if(systemEnv.userDefinedSetting.headerlink!=''){
					$(selector_header_banner).css('cursor','pointer');
					$(selector_header_banner).click(function(){
...
var DialogueHandler = {
    ...
    center : function(dialogueName){
        ...
        var _screenTop=(_clientHeight>_objHeight)?(_clientHeight-_objHeight)/2:0;
				if(!ToolsHandler.isMobile()){
					$(dialogueName).css('top',((_scrollTop+_screenTop)+'px'));
				}
...
var LanguageHandler = {
	...
	_lang = LanguageHandler.chkLang();
        $.each(systemEnv.LanguageSetting,function(i){
        	if(systemEnv.LanguageSetting[i].key==_lang){
        		_tmp1 = systemEnv.LanguageSetting[i].key;
        	}
        	if(systemEnv.LanguageSetting[i].selected){
        		_tmp2 = systemEnv.LanguageSetting[i].key;
        	}
        });
        if(_tmp2){
        	_currentlang = _tmp2;
	...
	APIHandler.setLang(_currentlang,function(data){});
	window.localStorage['lang'] = _currentlang;
    ...
    chkLang : function(){
    	_arrLang = ['zh-tw','zh-cn','jp','vi','en'];
    	lang = LanguageHandler.getLang();
    	if(_arrLang.indexOf(lang)>=0) return lang.toLowerCase();
    ...
    getLang : function(){
    	if(!window.localStorage['lang']){
    		window.localStorage['lang'] = 'zh-tw';
    	}
