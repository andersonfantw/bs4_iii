...
var MenuHandler = {
    ...
    doMenu : function(pre_text){
        ...
        for(i=0;i<bookEnv.MenuJSON.length;i++){
            $(selector_menu).append('<ul><li><a tabindex="'+bookEnv.MenuJSON[i].c_id+'">'+pre_text+bookEnv.MenuJSON[i].c_name+'</a></li></ul>');
            if(bookEnv.MenuJSON[i].sub_category!=null)
                for(j=0;j<bookEnv.MenuJSON[i].sub_category.length;j++){
                    $(selector_menu).find('ul li:last').append('<p><a tabindex="'+bookEnv.MenuJSON[i].sub_category[j].c_id+'">'+bookEnv.MenuJSON[i].sub_category[j].c_name+'</a></p>');
                }
        }
    }
}
...
var BooksHandler = {
    ...
    doSetBooks : function(books,book_type,catename){
        ...
        while(_index<books.length){
            ...
            if(bookEnv.InitJSON.member){
                ...
                var ibook_link = ToolsHandler.formatURL(books[_index].ibook_link.toString());
                var webbook_link = ToolsHandler.formatURL(books[_index].webbook_link.toString());
    ...
    doRemark : function(){
        var isShow = BooksHandler.isShowRemark();
    ...
    isShowRemark : function(){
    	return (parseInt($(selector_bookshelf).find('.cover > li > h3:first').css('height'))>0);
    },
    ...
    openBook : function(bookurl){
        ...
        APIHandler.logBookView(bookEnv.currentBookId, 0,bookEnv.openMode);
    },
        ...
        if(bookEnv.BooksDetailJSON!=null){
            ...
            if(bookEnv.InitJSON.member){
                ...
                if(bookEnv.BooksDetailJSON.ibook_show=="1"){ 
                    ...
                    $(selector_book_content).find('.button .ibook a').attr('href','javascript:BooksHandler.openBook("'+ToolsHandler.formatURL(bookEnv.BooksDetailJSON.ibook_link)+'");');
                }
                if(bookEnv.BooksDetailJSON.webbook_show=="1"){ 
                    $(selector_book_content).find('.button .webbook').show();
                    $(selector_book_content).find('.button .webbook a').attr('href','javascript:BooksHandler.openBook("'+ToolsHandler.formatURL(bookEnv.BooksDetailJSON.webbook_link)+'");');
                }
            }
...
var SearchHandler = {
    ...
    doSearch : function(){
        ...
        this.bindSearch(bookEnv.BooksBySearchJSON);
    ...
    bindSearch : function(data){
        ...
        for(i=0;i<data.length;i++){
            ...
            $result.find('.bookcontent').html(data[i].b_description);
...
var LoginHandler = {
    ...
    loginAndOpenBook : function(_pid,_pwd){
        ...
        APIHandler.loginAndOpenBook(_pid,_pwd,function(){
	        if(bookEnv.requestCode=='200'){
	          ...
	          APIHandler.logBookView(bookEnv.currentBookId,_pid,bookEnv.openMode);
                ...
                if(BooksHandler.mode()=='search'){
                    switch(MenuHandler.getFirstCate()){
                        ...
                        case 'new':
                            ...
                            break;
                        default:
                            $(selector_menu).find('ul:first li:first p').click();
                            break;
                    }
                    bookEnv.viewBookshelfMode();
                    return;
...
var TimerAction = function(){
    setInterval(BooksHandler.doRemark, 2000);
...
var MotionAction = function(){
    ...
    function Init(){
			$("#bookshelf").touchwipe({
				wipeLeft: function() { bookEnv.openBookRemark=false;BooksHandler.doRemark(); },
				wipeRight: function() { bookEnv.openBookRemark=true;BooksHandler.doRemark(); },
				wipeUp: function() {},
				wipeDown: function() {},
				min_move_x: 20,
				min_move_y: 20,
...
$(document).ready(function(){
  ...
  //for list
  $('#header .logout').text(systemEnv.Language.logout);
});
