...
switch ($type) {
  ...
  case 'edit':
		if(is_numeric($id)){
			...
			$data = $allexam->getByKey($url['key'],$url['date']);
		}
  	$tpl->assign('data',$data);
    $tpl->display('backend/sys_allexam_edit.tpl');
    break;
  case 'do_update':
		$status->go('sys_allexam.php?page='.$page.'&q='.$q_str,'success',LANG_MESSAGE_UPDATE_SUCCESS);
    break;
  case 'delete':
  	parse_str(base64_decode($id),$url);
  ...
  default:
    ...
    switch($type){
    	case 'search_instant':
      	...
      	$tpl->display('backend/sys_allexam_list_data.tpl');
      	exit;
      	break;
    }
    $record=$data['total'];
    ...
    $tpl->assign('data',$data['result']);
    $tpl->assign('sys_allexam_list_data_html', $tpl->fetch('backend/sys_allexam_list_data.tpl'));
    $tpl->display('backend/sys_allexam_list.tpl');
    break;
}
?>